...
AVAILABLE_TEMPLATES = ["gonmun", "report", "minutes", "nrf_proposal"]
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if template:
            ...
            for overlay_file in overlay_dir.iterdir():
                if overlay_file.is_file() and overlay_file.suffix in (".xml", ".hpf"):
                    # .xml and .hpf files go into Contents/
                    dest = work / "Contents" / overlay_file.name
                    shutil.copy2(overlay_file, dest)
                elif overlay_file.is_dir():
                    # Subdirectories (e.g. BinData/) are copied to the work root
                    dest_dir = work / overlay_file.name
                    if dest_dir.exists():
                        shutil.rmtree(dest_dir)
                    shutil.copytree(overlay_file, dest_dir)
