# PDF to Markdown Converter (MinerU)

PDF 파일을 MinerU로 변환하고 Obsidian 프론트매터를 자동 삽입한다.
수식은 LaTeX, 그림은 이미지 파일, 테이블은 HTML로 변환된다.
...
```
python "C:/Users/JHKIM/my-tools/plugins/mineru-tools/convert_mineru.py" "파일경로" [-o 출력디렉토리]
```
...
| 파일 | 상태 | 출력 경로 | 이미지 |
|------|------|----------|--------|
| input.pdf | 성공/실패 | output.md | N개 추출 |
