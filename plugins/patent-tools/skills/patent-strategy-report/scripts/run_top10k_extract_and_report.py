# -*- coding: utf-8 -*-
"""Extract top 10k by title–RFP relevance and generate 10k report (paths resolved in-process)."""
import sys
import json
import subprocess
...
v1_top10000_csv = out_dir / "v1_top10000.csv"

if not v1_csv.exists():
    print("v1 CSV not found:", v1_csv, file=sys.stderr)
    sys.exit(1)
...
n = run_filter(str(v1_csv), str(rfp), str(v1_top10000_csv), 10000)
print("Wrote top 10,000:", v1_top10000_csv)

# Aggregate in-process (same pattern as run_aggregate_5000_inprocess)
...
rows, _ = load_csv(str(v1_top10000_csv))
total = len(rows)
...
out = {
    ...
    "search_query": "(stretchable AND display AND …) [제목-RFP 연관성 상위 10,000건]",
...
report_md = out_dir / "20260307_센서융합디스플레이_10000건_세계특허현황_분석보고서.md"
r = subprocess.run(
    [
