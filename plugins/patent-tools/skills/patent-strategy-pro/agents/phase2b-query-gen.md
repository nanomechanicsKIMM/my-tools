...
#### 5A-2. EPO 건수 조정 루프 (최대 5회)

EPO 모드에서도 검색 건수를 확인하고 목표 범위에 맞게 CQL 쿼리를 조정한다.
`search_patents_epo.py --count-only` 모드를 사용하여 실제 다운로드 없이 건수만 빠르게 확인한다.

**목표 건수**: MAIN ~2,000건 (EPO 한도) / SUB ~500건

```
FOR each query (main CQL, sub1 CQL, sub2 CQL, ...):
  iteration = 0
  WHILE iteration < 5:
    1. CQL 쿼리 구성 (generate_query.py의 --format cql 출력 또는 수동 구성)
    2. 건수 확인:
       python "{scripts_dir}/search_patents_epo.py" --cql "{cql}" --count-only -o /dev/null
       → JSON 출력: {"cql": "...", "total": N}
    3. 건수 판정:
       - MAIN 범위 내 (1,000~2,000) / SUB 범위 내 (200~800) → ✅ 확정
       - 건수 >> 목표 → AND 그룹 추가, ti= (제목 한정)으로 축소
       - 건수 > 목표 (1.2~2배) → OR 그룹에서 넓은 키워드 제거
       - 건수 < 목표 (0.5배 미만) → AND 그룹 제거, ta= 검색으로 전환
       - 413 에러 (total=-1) → OR 그룹 분할, 개별 키워드 검색으로 전환
    4. CQL 수정 후 iteration += 1

  IF iteration == 5 AND still out of range:
    현재 CQL로 확정, 경고 메시지 기록
```

#### 5A-3. 실제 다운로드 실행

건수 조정 완료 후 확정된 CQL로 실제 다운로드를 실행한다:
...
python "{scripts_dir}/search_patents_epo.py" \
  ...
  --years {years} \
  --split-by-year \
  -o "{output_dir}/gp-search-{date}_main.csv"
...
`queries_sub_techs.md`에 EPO 검증 결과 테이블을 기록:
```markdown
> [!success] EPO OPS 자동 검증 결과 ({date})
> | 검색식 | CQL 건수 | 다운로드 건수 | 목표 | 상태 |
> |--------|---------|-------------|------|------|
> | MAIN | {count_total} | {csv_rows} | ~2,000 | ✅/⚠️ |
> | SUB1 | {count_total} | {csv_rows} | ~500 | ✅/⚠️ |
> ...
```
