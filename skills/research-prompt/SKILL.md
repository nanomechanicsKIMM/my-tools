...
description: Write a single-paragraph Deep Research prompt to hand to a human researcher (or a deep-research AI). Use when the user wants a research brief, a "deep research prompt", a one-paragraph task for a researcher, or asks "what should our researcher look for". Produces ONE tight paragraph with full context, numbered sub-questions, and per-finding output format. 국문 트리거: "리서치 브리프", "조사 프롬프트", "선행조사 계획서", "무엇을 조사할지", "딥리서치 프롬프트".
---

<!-- Source: davidondrej/skills · research-and-web/research-prompt (MIT License, © 2026 David Ondrej). Vendored into my-tools; execution section adapted for LLM_wiki. Full license: ../_shared/THIRD-PARTY.md -->
...
This skill produces the prompt only. To run it, hand the paragraph to a deep-research tool available in this environment — the `/deep-research` harness, a WebSearch/WebFetch fan-out, or the daily paper-recommender pipeline. One mission per run. Save the cited output as a markdown report under `LLM_work/report/` (`(YYYYMMDD) 제목.md`), listing source links beneath the findings.
