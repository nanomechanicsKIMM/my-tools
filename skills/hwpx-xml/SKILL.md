---
name: hwpx-xml
description: "HWPX 문서를 XML 직접 작성 방식으로 생성·편집하는 스킬. python-hwpx API 서식 버그를 우회하여 세밀한 서식 제어가 필요할 때 사용. 템플릿: base/gonmun/report/minutes/proposal. 스크립트: build_hwpx.py(조립), analyze_template.py(분석), unpack/pack(편집), validate.py(검증). lxml 패키지 필요."
---
