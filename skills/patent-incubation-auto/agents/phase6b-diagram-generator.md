...
표기 금지. §9에는 도면 번호([도 N])·번호별 설명 목록을 작성하지 않고 각 도면에 내용 기반
제목만 단다. 사유: 출원용 정식 도면·부호 체계는 변리사가 별도 작성 — 내부 번호가 오해 유발.
...
4. **폴백**(COM 불가 환경만): python-pptx + 고해상 PNG 삽입 — 이 경우 산출 보고에
   ...
   > PowerPoint COM `SaveAs`는 한글·괄호가 포함된 출력 경로에서 COMException으로 실패하거나
   > 무한 대기(모달)할 수 있다. **`$pp.DisplayAlerts = 1`(ppAlertsNone) 설정 + ASCII 임시
   > 경로(예: `%TEMP%\deck_tmp.pptx`)에 `SaveAs(path, 24)` 후 최종 위치로 move** 방식을
   > 기본으로 한다. 실패 시 잔류 POWERPNT 프로세스를 kill 후 재시도(COM 클래스 점유 해제).
...
- **캡션 파일 필수 (2026-07-13)**: `diagrams/captions.json`에 `{"figN_*.png": "내용 기반 제목"}`
  맵을 함께 생성한다 — convert_hwpx.py가 이를 읽어 §9 캡션(제목만, 번호 없음)으로 삽입한다.
  파일이 없으면 파일명 stem이 캡션이 되므로 반드시 생성.
...
- **도면 번호·부호 미사용 (2026-07-13)**: 문서·캡션에 `[도 N]` 번호를 노출하지 않고, 도면 내
  구성요소 라벨은 **부품 이름 텍스트만** 사용(참조 부호 10·100 등 금지) — 출원용 정식 도면은
  변리사가 별도 작성하므로 내부 번호가 오해 유발. 도면 순서는 파일명 `fig1_`, `fig2_` 접두사로만
  관리하며 **figures_deck.pptx 슬라이드 순서 = 도면 순서 1:1**(표지 슬라이드 금지).
