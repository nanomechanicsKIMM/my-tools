...
- **`rebalance_policy.py` — 리밸런싱 정책 비교(P2-1, v2)**: 캘린더(3/6/12M+오프셋)·밴드(±3/5/7%p)·**부분 복원**(`band5_d50/_d875` — Vanguard 2024 'The Rebalancing Edge' 200/175식, 목표±밴드×dest 지점까지만 복원)·**현금흐름**(부담금→언더웨이트, 매도 없음) 2버킷 시뮬 + 타이밍럭·비용/지연 민감도. 실증: ①고갈확률 전 정책 무차이·회전율 4배차 → **현금흐름+밴드5 채택**(IPS §3) ②부분 복원 d875는 회전율 6.4→5.4%/y 추가 절감(고갈 무차이) → 안전장치 복원 목적지 채택 ③캘린더 오프셋 타이밍럭(Hoffstein 2020) 고갈 ≤0.02%p — 결론 강건 ④비용 0↔0.2%·T+8 지연 무영향. 문헌: Daryanani 2008·Vanguard 2022/2024·Swedroe 5/25. ⚠ 2버킷 한계 — 펀드 레벨 5/25는 ips_check/drift_check 소관, 골드·신흥국 지수 부재로 7버킷 시뮬 불가. 상세 §16.
...
- **`ips_check.py` — IPS 자동 대조(P3-1)**: `status/ips_policy.json`(기계판독 정책, `templates/ips.md` 쌍) vs 보유내역 6항목 판정(위험·단일·밴드·펀드수·TER·신흥국). 밴드는 **5/25 규칙**(임계=min(절대 band_pp, 목표×band_rel_frac), 하한 0.5%p) — 절대 ±5%p 단독은 4~7% 위성 포지션에 트리거 불능(인도 4%가 9%까지 침묵)이라 상대밴드로 보완(Daryanani 2008·Swedroe 5/25). `band_rel_frac` 없으면 종전 절대 밴드(하위호환). FAIL=전환 미완 신호. 상세 §17.
...
- **`drift_check.py` — 보유 드리프트 점검**: 사용자 보유내역을 `status/holdings_YYYYMMDD.json`(`{"asof","holdings":[{"name","code","value","kind":"fund|deposit|cash"}]}`)으로 구조화한 뒤 실행. 비중표·위험자산비중·DC 한도(위험70/단일40) 판정, `--targets` JSON(코드→비중%)을 주면 목표 대비 괴리(**5/25 밴드이탈 플래그**, `--band-pp`/`--band-rel` 조정)까지 출력. 분기 완주와 별개로 **월 1회 경량 look** 용도("자주 보되 드물게 거래" — Daryanani). ⚠ '기타' 분류(골드·TDF)가 안전으로 집계되어 포털 공시 위험비중과 다를 수 있음 — 포털값 병기 확인.
