...
> `~/my-tools/skills/patent-strategy-report/scripts/` 폴더가 존재하면 이 섹션을 건너뛰고 **§2 사전 준비**부터 시작합니다.
...
원하는 위치에 저장소를 클론합니다. 아래는 홈 디렉토리(`~`) 아래에 받는 예시입니다.
...
> 이 경우 스크립트 경로를 `~/my-tools/skills/patent-strategy-report/scripts/`로 직접 지정하면 됩니다.
...
```powershell
cd ~/my-tools/skills/patent-strategy-report/scripts
uv venv
...
| 항목 | 설명 | 예시 |
|------|------|------|
| **RFP 파일 경로** | 분석 대상 RFP 마크다운 파일 | `~/patent_b/RFP_신축디스플레이.md` |
| **CSV 파일 경로** | Google Patents에서 받은 CSV | `~/patent_b/patents.csv` |
...
```bash
PYTHONUTF8=1 python "~/my-tools/skills/patent-strategy-report/scripts/generate_query.py" \
  "<RFP 파일 경로>" \
...
```bash
PYTHONUTF8=1 python "~/my-tools/skills/patent-strategy-report/scripts/run_relevance_pipeline.py" \
  "<CSV 파일 경로>" "<RFP 파일 경로>" \
  -o "<출력 폴더 경로>" \
