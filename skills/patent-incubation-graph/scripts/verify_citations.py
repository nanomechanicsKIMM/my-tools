...
# 외국 특허번호(국가코드+숫자+종류코드), 예: CN114950925A, US10573627B2, WO2020016250A1
FOREIGN_RE = re.compile(r"\b[A-Z]{2}\d{5,}[A-Z]\d?\b")
...
def extract_keys(text: str) -> set[str]:
    ...
    for k in FOREIGN_RE.findall(text):
        keys.add("foreign:" + k.upper())
    return keys
...
def main() -> int:
    ...
    for c in vj.get("citations", []):
        ...
        # status는 citation 최상위(status) 또는 문서 예시 스키마의 verification.status
        # 둘 중 어디에 있어도 인식한다.
        st = c.get("status") or c.get("verification", {}).get("status")
        if st in BACKED:
            verified_keys |= ks
        elif st in REMOVED:
            removed_keys |= ks
